...
const mod = new Function('supabase', 'weekId', 'LEAGUES', 'estimateTOEICScore',
  src + '\nreturn { applyWeekTransition, pushWeeklySnapshot };')(supabase, weekId, [], () => ({ total: null }));
...
setTimeout(() => {
  ...
  ok('week_start = lundi de la semaine finie, au format date', /^\d{4}-\d{2}-\d{2}$/.test(p.week_start || ''));
