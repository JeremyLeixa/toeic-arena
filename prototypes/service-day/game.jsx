// Proto « monde service client » (2026-09-25) — moteur de prototypes/travel-day/ (celui de Nine to Five, V3 :
// horloge + réputation). Un seul élément propre au monde, en deux variantes (paramètre s) :
//   s=1 Note client    : chaque client servi laisse une note (5 étoiles si tout est juste, une de moins par erreur,
//                        1 s'il est reparti sans réponse) ; la moyenne s'affiche en haut. De l'affichage, rien d'autre.
//   s=2 Briefé = calme : le point du matin porte sur les clients difficiles. Compris en entier, le client mécontent
//                        de l'après-midi repart calmé (+15 rep) ; sinon il faut aller chercher la responsable (+45 min).
//                        C'est la règle W2 de Jet Lag, telle quelle : aucun code moteur neuf au câblage.
// Les questions sont celles du TOEIC, telles quelles : tout le jeu est autour, jamais à leur place.
import { useEffect, useMemo, useRef, useState } from "react";
import { COMPANY, DAY, DAY_LEN, DAY_START, GRADES, PEOPLE, REP_PER_CORRECT, REP_PER_TASK, CALM_BONUS, CALM_DELAY } from "./day.js";
...
var KIND = {
  ...
  briefing: { icon: "👥", label: "Huddle" },
};
// Libellé du bouton quand ça sonne.
function pickLabel(t) { return t.kind === "call" ? "Pick up" : t.kind === "counter" ? "Serve" : "Listen"; }
// Variante S1 : la note d'un client. Tout juste = 5, une étoile de moins par erreur (plancher 2), reparti sans réponse = 1.
function rating(t, s) {
  if (s.status === "missed") return 1;
  if (s.status !== "done") return null;
  var ko = t.qs.filter(function (q, k) { return s.answers[k] !== q.c; }).length;
  return Math.max(2, 5 - ko);
}
function avgRating(st) {
  var r = DAY.tasks.filter(function (t) { return t.customer; }).map(function (t) { return rating(t, st[t.id]); }).filter(function (x) { return x != null; });
  return r.length ? r.reduce(function (a, b) { return a + b; }, 0) / r.length : null;
}
function starsTxt(n) { return "★★★★★".slice(0, n) + "☆☆☆☆☆".slice(0, 5 - n); }
...
function simulatedEnd() {
  // start=end : 5 tâches sur 6, le client du comptoir reparti sans réponse.
  var s = initialState();
  var picks = {};
  DAY.tasks.forEach(function (t) { if (t.id !== "t3") picks[t.id] = t.qs.map(function (q, k) { return k === 1 ? (q.c + 1) % q.opts.length : q.c; }); });
  var done = { t1: 30, t2: 120, t4: 175, t5: 230, t6: 400 };
  DAY.tasks.forEach(function (t) {
...
export function OfficeDay(p) {
  ...
  var [min, setMin] = useState(p.start === "end" ? DAY_LEN : 0);
  var [st, setSt] = useState(p.start === "end" ? simulatedEnd : initialState);
  var [active, setActive] = useState(null);
  var [toast, setToast] = useState(null);
  // Point du matin : entendu une fois traité ; paré = compris en entier ; bonus / retard de la variante 2.
  var [wx, setWx] = useState(p.start === "end" ? { known: true, prepared: false, bonus: 0, hit: "caught" } : { known: false, prepared: false, bonus: 0, hit: null });
  ...
  useEffect(function () {
    ...
    // Variante 2 : le client mécontent appelle. Briefé (point du matin compris) → il repart calmé, bonus ;
    // sinon → il demande la responsable, l'horloge saute. Une seule fois (wx.hit posé).
    var hit = arrivals.find(function (x) { return x.angryHit; });
    if (hit && p.svc === "2" && !wxRef.current.hit) {
      if (wxRef.current.prepared) {
        setWx(function (w) { return Object.assign({}, w, { hit: "ready", bonus: CALM_BONUS }); });
        say("👥 An upset customer on the line. You know what to do · +" + CALM_BONUS + " rep");
      } else {
        setWx(function (w) { return Object.assign({}, w, { hit: "caught" }); });
        if (clock) setMin(function (m) { return Math.min(DAY_LEN, m + CALM_DELAY); });
        say("👥 An upset customer asks for the manager · +" + CALM_DELAY + " min");
      }
      return;
    }
    ...
    if (t) say(t.kind === "doc" ? "New " + t.docType.toLowerCase() + " · " + t.subject : t.kind === "briefing" ? "👥 The morning huddle is starting" : t.kind === "announce" ? "📢 " + t.from : t.kind === "counter" ? "🛎 A customer at the counter" : "☎ " + t.from);
  ...
  function next(id) {
    ...
    if (t.briefing) {
      ...
      say("👥 Huddle over" + (p.svc === "2" ? (prepared ? " · you know what to do" : " · some advice slipped past you") : ""));
      return;
    }
    if (t.customer && p.svc === "1") {
      say(starsTxt(Math.max(2, 5 - (t.qs.length - ok))) + " · the customer rated you" + (late ? " · late" : ""));
      return;
    }
  ...
  return (
    <div className={"od od-v" + p.v}>
      {phase !== "intro" && phase !== "end" && <TopBar clock={clock} career={career} min={min} rep={rep} wx={wx} svc={p.svc} avg={avgRating(st)} />}
      {phase === "intro" && <Intro clock={clock} career={career} speed={p.speed} svc={p.svc} onStart={function () { setPhase("desk"); }} />}
      ...
      {phase === "end" && <EndOfDay st={st} clock={clock} career={career} min={min} wx={wx} svc={p.svc} onAgain={function () { location.reload(); }} />}
...
function TopBar(p) {
  ...
  return (
    <div className="od-top">
      ...
      {p.svc === "1"
        ? <div className={"od-wx" + (p.avg != null ? "" : " od-wx-unk")} title="Customer rating">{p.avg != null ? "★ " + p.avg.toFixed(1) : "★ –"}</div>
        : <div className={"od-wx" + (p.wx.known ? "" : " od-wx-unk")} title="Morning huddle">{p.wx.known ? (p.wx.prepared ? "👥 Briefed" : "👥 Half-briefed") : "👥 ?"}</div>}
    ...
    <div className="od-intro">
      ...
      <ul className="od-rules">
        {p.clock ? <>
          ...
          <li><b>Customers don't wait.</b> Neither does the phone.</li>
          {p.svc === "2"
            ? <li>Listen to the huddle carefully: <b>a difficult customer is coming.</b></li>
            : <li>Every customer you serve <b>rates you</b>, from 1 to 5 stars.</li>}
        </> : <>
...
function Desk(p) {
  ...
  return (
    <div className="od-desk">
      <div className="od-sec">Your counter</div>
      {visible.length === 0 && (
        <div className="od-empty">
          <p>{pending ? "Nothing to handle right now." : "All clear."}</p>
          {p.clock && pending && <button className="od-btn" onClick={p.onSkip}>Tidy the shelves · skip to the next customer</button>}
...
function EndOfDay(p) {
  ...
  return (
    <div className="od-end">
      ...
      {p.svc === "2" && p.wx.hit && <div className="od-review od-wx-line"><p>{p.wx.hit === "ready" ? "👥 The upset customer left calm: you listened first, like the huddle said. +" + CALM_BONUS + " rep." : "👥 The upset customer asked for the manager: " + CALM_DELAY + " minutes lost. The huddle had the answer."}</p></div>}
      {p.svc === "1" && avg != null && <div className="od-review od-wx-line"><p>Customer rating today: <b>{"★ " + avg.toFixed(1) + " / 5"}</b></p></div>}
        ...
        {rows.map(function (r) {
          return (
            <div key={r.t.id} className={"od-end-row od-st-" + r.status.replace(/ /g, "-")}>
              ...
              <span className="od-chip">{r.answered ? r.ok + "/" + r.t.qs.length : ""}{r.status !== "done" ? (r.answered ? " · " : "") + r.status : ""}{p.svc === "1" && r.t.customer && rating(r.t, p.st[r.t.id]) != null ? " · " + starsTxt(rating(r.t, p.st[r.t.id])) : ""}</span>
            </div>
          );
        })}
      </div>
