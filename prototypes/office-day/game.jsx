// Proto « journée au bureau » — le jeu.
//   v=1 Rush     : horloge 9:00 → 17:00, arrivées à l'heure, appels qui n'attendent pas, échéances.
//   v=2 Carrière : pas d'horloge, les tâches arrivent au fil de l'eau, réputation et promotion.
//   v=3 Les deux : horloge + réputation (bonus « à l'heure »).
// Les questions sont celles du TOEIC, telles quelles : tout le jeu est autour, jamais à leur place.
import { useEffect, useMemo, useRef, useState } from "react";
import { COMPANY, DAY, DAY_LEN, DAY_START, GRADES, PEOPLE, REP_PER_CORRECT, REP_PER_TASK } from "./day.js";
...
function simulatedEnd() {
  // start=end : 4 tâches sur 5, l'appel de 14:00 manqué, la note du pique-nique rendue en retard.
  var s = initialState();
  // 12/16 et 4 tâches rendues : 180 + 72 = 252 rep, juste de quoi montrer la promotion.
  var picks = { t1: [1, 3, 1, 0], t2: [1, 2, 1], t3: [1, 2, 1], t4: [1, 2, 1] };
  var done = { t1: 150, t2: 470, t3: 75, t4: 330 };
  DAY.tasks.forEach(function (t) {
...
export function OfficeDay(p) {
  ...
  useEffect(function () {
    ...
    if (t) say(t.kind === "voicemail" ? "New voicemail · " + t.from.split(" · ")[0] : t.kind === "doc" ? "New " + t.docType.toLowerCase() + " · " + t.subject : t.kind === "chat" ? "☕ " + t.subject + " · at the coffee machine" : "☎ Incoming · " + t.from.split(" · ")[0]);
    else if (lost.length) say((lost[0].kind === "chat" ? "The coffee break is over · " : "Missed call · ") + lost[0].subject);
  ...
  function next(id) {
    ...
    stopAudio(); setActive(null); setPhase("desk");
    say("Filed · " + ok + "/" + t.qs.length + (late ? " · late" : "") + (career ? " · +" + (ok * REP_PER_CORRECT + (late ? 0 : REP_PER_TASK)) + " rep" : ""));
  }
  function skipAhead() {
  ...
  var rep = REP_START + repGain(st, clock);

  return (
    <div className={"od od-v" + p.v}>
      {phase !== "intro" && phase !== "end" && <TopBar clock={clock} career={career} min={min} rep={rep} />}
      {phase === "intro" && <Intro clock={clock} career={career} speed={p.speed} onStart={function () { setPhase("desk"); }} />}
      ...
      {phase === "end" && <EndOfDay st={st} clock={clock} career={career} min={min} onAgain={function () { location.reload(); }} />}
...
function TopBar(p) {
  ...
  return (
    <div className="od-top">
      <div className="od-co"><span className="od-logo">M</span><span>{COMPANY}</span></div>
    ...
    <div className="od-intro">
      ...
      <ul className="od-rules">
        {p.clock ? <>
          ...
          <li>Work arrives during the day. <b>Calls don't wait.</b></li>
...
function Desk(p) {
  ...
  return (
    <div className="od-desk">
      <div className="od-sec">On your desk</div>
      {visible.length === 0 && (
        <div className="od-empty">
          <p>{pending ? "Nothing on your desk right now." : "All clear."}</p>
          {p.clock && pending && <button className="od-btn" onClick={p.onSkip}>Grab a coffee · wait for the next task</button>}
  ...
  return (
    <button className={"od-row" + (ring ? " od-row-ring" : "")} onClick={p.onOpen}>
      ...
      <div className="od-row-r">
        {ring && <span className="od-chip od-chip-ring">{t.kind === "chat" ? "Join" : "Pick up"}{left != null ? " · " + left + " min" : ""}</span>}
...
function RingBanner(p) {
  ...
  return (
    <div className="od-ring">
      ...
      <div className="od-ring-b"><b>{p.t.kind === "chat" ? "Coffee machine" : p.t.from.split(" · ")[0]}</b><span>{p.t.subject}{left != null ? " · " + left + " min" : ""}</span></div>
      <button className="od-btn od-btn-ring" disabled={p.busy} onClick={p.onPick}>{p.busy ? "Busy" : p.t.kind === "chat" ? "Join" : "Pick up"}</button>
    </div>
  );
}
    ...
    <div className="od-task">
      ...
      {t.kind === "doc" ? <Document t={t} /> : (
        <div className={"od-call od-call-" + t.kind}>
          <div className="od-call-who">{t.kind === "chat" ? "☕ Coffee machine" : t.from}</div>
