// ═══════════════════════════════════════════════════════════════
// particles.js — un seul <canvas> pour toutes les particules
// DPR plafonné à 2, quantités divisées par ~2 sur les appareils à ≤4 cœurs,
// boucle rAF arrêtée dès qu'il n'y a plus rien à dessiner.
// ═══════════════════════════════════════════════════════════════
(function () {
  var cv, g, W = 0, H = 0, parts = [], raf = 0, lite = false, reduced = false;

  function attach(canvas) {
    cv = canvas; g = cv.getContext("2d");
    lite = (navigator.hardwareConcurrency || 4) <= 4;
  ...
  function n(count) { return reduced ? 0 : Math.max(1, Math.round(count * (lite ? 0.5 : 1))); }
  function start() { if (!raf) raf = requestAnimationFrame(loop); }

  // o = {x,y,count,color,angle(deg, 0 = haut),spread,speed:[a,b],gravity,drag,life:[a,b],size:[a,b],kind}
  function emit(o) {
    var c = n(o.count || 10);
    for (var i = 0; i < c; i++) {
      ...
      parts.push({
        ...
        g: o.gravity || 0, drag: o.drag || 0.97, life: life, max: life,
        size: rand(o.size ? o.size[0] : 1, o.size ? o.size[1] : 3),
  ...
  function attract(o) {
    var c = n(o.count || 20);
    for (var i = 0; i < c; i++) {
      var a = Math.random() * Math.PI * 2, r = rand(o.r * 0.7, o.r * 1.2);
      var life = rand(30, 50);
      parts.push({
        x: o.x + Math.cos(a) * r, y: o.y + Math.sin(a) * r * 0.75, vx: 0, vy: 0,
        tx: o.x, ty: o.y, life: life, max: life, size: rand(1, 2.6),
        color: pickc(o.color), kind: "attract", g: 0, drag: 0.9, tw: 0,
      });
    }
    start();
  }
  // Pluie de poussière d'or sur toute la largeur (légendaire)
  function rain(o) {
    var c = n(o.count || 40);
    for (var i = 0; i < c; i++) {
      var life = rand(90, 160);
      parts.push({
        x: rand(0, W), y: rand(-H * 0.4, -10), vx: rand(-0.3, 0.3), vy: rand(0.6, 1.8),
        g: 0.01, drag: 0.995, life: life, max: life, size: rand(1, 2.8),
        color: pickc(o.color), kind: "ember", tw: Math.random() * 6,
      });
    }
    start();
  }
  ...
  function loop() {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (p.kind === "dust") {
        ...
        // spark : traînée orientée selon la vitesse
        g.globalCompositeOperation = "lighter";
  ...
  window.FX = {
    attach: attach, resize: resize, emit: emit, attract: attract, rain: rain,
    clear: function () { parts = []; if (g) g.clearRect(0, 0, W, H); },
