// Proto « monde voyage » (2026-09-25) — copie du moteur de prototypes/office-day/ (variante V3 retenue pour Nine to Five :
// horloge + réputation), avec la météo en plus. Deux variantes de météo, paramètre w :
//   w=1 Bulletin      : le bulletin du matin est une tâche comme une autre ; la météo s'affiche en haut une fois entendue.
//   w=2 Prévu = paré  : si le bulletin est compris (toutes les réponses justes), l'annonce de retard trouve l'élève paré
//                       (rebooké à temps, +15 rep) ; sinon il reste coincé à la porte (+45 min sur l'horloge).
// Les questions sont celles du TOEIC, telles quelles : tout le jeu est autour, jamais à leur place.
import { useEffect, useMemo, useRef, useState } from "react";
import { COMPANY, DAY, DAY_LEN, DAY_START, GRADES, PEOPLE, REP_PER_CORRECT, REP_PER_TASK, WEATHER_BONUS, WEATHER_DELAY } from "./day.js";
...
var KIND = {
  ...
  forecast: { icon: "☔", label: "Weather" },
};
// Libellé du bouton quand ça sonne.
function pickLabel(t) { return t.kind === "call" ? "Pick up" : "Listen"; }
...
function simulatedEnd() {
  // start=end : 5 tâches sur 6, la conversation du tableau des départs manquée.
  var s = initialState();
  var picks = {};
  DAY.tasks.forEach(function (t) { if (t.id !== "t5") picks[t.id] = t.qs.map(function (q, k) { return k === 1 ? (q.c + 1) % q.opts.length : q.c; }); });
  var done = { t1: 30, t2: 120, t3: 90, t4: 175, t6: 400 };
  DAY.tasks.forEach(function (t) {
...
export function OfficeDay(p) {
  ...
  // Météo : connue une fois le bulletin traité ; paré = bulletin compris en entier ; bonus / retard de la variante 2.
  ...
  useEffect(function () {
    ...
    // Variante 2 : l'annonce de retard due à la météo tombe. Paré (bulletin compris) → rebooké à temps, bonus ;
    // sinon → coincé à la porte, l'horloge saute. Une seule fois (wx.hit posé).
    var hit = arrivals.find(function (x) { return x.weatherHit; });
    if (hit && p.weather === "2" && !wxRef.current.hit) {
      if (wxRef.current.prepared) {
        setWx(function (w) { return Object.assign({}, w, { hit: "ready", bonus: WEATHER_BONUS }); });
        say("☔ You saw it coming: rebooked on an earlier flight · +" + WEATHER_BONUS + " rep");
      } else {
        setWx(function (w) { return Object.assign({}, w, { hit: "caught" }); });
        if (clock) setMin(function (m) { return Math.min(DAY_LEN, m + WEATHER_DELAY); });
        say("☔ Caught by the storm · stuck at the gate, +" + WEATHER_DELAY + " min");
      }
      return;
    }
    ...
    if (t) say(t.kind === "doc" ? "New " + t.docType.toLowerCase() + " · " + t.subject : t.kind === "forecast" ? "☔ Weather forecast on the radio" : t.kind === "announce" ? "📢 " + t.from : t.kind === "chat" ? "💬 " + t.subject : "☎ " + t.from);
  ...
  function next(id) {
    ...
    if (t.forecast) {
      ...
      say("☔ " + DAY.forecastLabel + " today" + (p.weather === "2" ? (prepared ? " · you're ready for it" : " · some details slipped past you") : ""));
      return;
    }
  ...
  return (
    <div className={"od od-v" + p.v}>
      {phase !== "intro" && phase !== "end" && <TopBar clock={clock} career={career} min={min} rep={rep} wx={wx} />}
      {phase === "intro" && <Intro clock={clock} career={career} speed={p.speed} weather={p.weather} onStart={function () { setPhase("desk"); }} />}
      ...
      {phase === "end" && <EndOfDay st={st} clock={clock} career={career} min={min} wx={wx} weather={p.weather} onAgain={function () { location.reload(); }} />}
...
function TopBar(p) {
  ...
  return (
    <div className="od-top">
      ...
      <div className={"od-wx" + (p.wx.known ? "" : " od-wx-unk")} title="Weather">{p.wx.known ? "☔ " + DAY.forecastLabel : "☁ ?"}</div>
    ...
    <div className="od-intro">
      ...
      <ul className="od-rules">
        {p.clock ? <>
          ...
          <li><b>Announcements don't wait.</b> Neither do taxis.</li>
          {p.weather === "2"
            ? <li>Listen to the forecast carefully: <b>the weather can change your trip.</b></li>
...
function Desk(p) {
  ...
  return (
    <div className="od-desk">
      <div className="od-sec">Your trip</div>
      {visible.length === 0 && (
        <div className="od-empty">
          <p>{pending ? "Nothing to handle right now." : "All clear."}</p>
          {p.clock && pending && <button className="od-btn" onClick={p.onSkip}>Wait in the lounge · skip to the next event</button>}
...
function EndOfDay(p) {
  ...
  return (
    <div className="od-end">
      ...
      {p.weather === "2" && p.wx.hit && <div className="od-review od-wx-line"><p>{p.wx.hit === "ready" ? "☔ You read the forecast right and rebooked in time: +" + WEATHER_BONUS + " rep." : "☔ The storm caught you at the gate: " + WEATHER_DELAY + " minutes lost. The forecast had warned you."}</p></div>}
