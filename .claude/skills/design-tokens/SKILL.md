...
- **League / title / rarity colors from data** (`lg.color`, `TITLES[id].color`, `rarity.color` from `RARITIES`, `shopRarColor()`) are bright hexes tuned for dark mode : washed out in light mode (Gold `#ffd700` at 1.07:1, Legendary `#ffc020` at 1.25:1). Always render them through `tone(hex)` from `src/lib/tone.js` (light variant defined in `.light{--tone-<hex>}`, `appCss.js`). A new league, title or rarity color needs its variant + an `initial` reset in the cartes-nuit rule. Raw hex only on a fixed dark background (`#1a1208`). Guarded by `tests/check_tones.cjs`.
