...
- **Any hardcoded text/icon hex** (league, title, rarity colors, but also accent hexes in JSX and data : `#f0c850`, `#7fb8e8`, `#22c55e`…) is tuned for dark mode and washes out in light mode (1.0-2.9:1). Render it through `tone("#rrggbb")` from `src/lib/tone.js` (light variant in `.light{--tone-<hex>}`, `appCss.js`, + an `initial` reset in the cartes-nuit rule). Data-driven color (`color:x.col`) → `tone(x.col)` + declare the source in `DATA_SOURCES` of `tests/check_tones.cjs`. Light text on a **hardcoded dark background** set elsewhere (dark modal, Endless banner, Boss/Endless tile) : keep the hex, prefix it `/*fond local*/`, and **never use a theme token (`var(--gold)`, `var(--t3)`) inside such a block** — it follows light mode while the background doesn't (freeze the dark-mode value). The guard refuses any hex under 4.5:1 (AA) on light backgrounds otherwise (a same-line background counts only if it gives 4.5:1). The dark-mode `--t2` gray `#8a7e6a` written as a literal is no exception: `tone("#8a7e6a")` (light variant = light `--t2` `#5a5040`).
- **Text on an accent fill** (`--cx-hex`/`--cyan` background : `.btn1`, level medal, badges) : `color: var(--on-cx)` (dark in dark mode, `#fffcf5` in light where the accent is darkened). Never hardcode `#0f0c08` there : 1.6-2.5:1 in light.
- **Light-mode token values** (2026-09-16) : default Doré `--cyan`/`--cx-hex` `#6f5410` (`--cx-dark` `#55400c`), `--gold` `#7c5d0e`, `--orange` `#834a0d`, `--green` `#106430` (all ≥ 4.6:1 on `--bg`/`--bg2`/`--bg3`). Darkening a light token can break text on a hardcoded dark surface that uses it (Boss tile, achievement/chest toasts) : check those after any change.
