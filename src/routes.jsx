...
import { Drill, GrammarRef, LinkingBridge, PhrasalDojo, TrapsQuiz, WordFam } from "./features/train/grammar.jsx";
...
export function renderRoute(c){
  ...
  if(sp==="tavern"){if(!lastSession)playBGM("bgm_tavern");return pg(<WordTavern u={u} nav={nav} done={function(sc,tot,xp,mistakes,extra){stopBGM();return miniSession(sc,tot,xp,mistakes,extra);}} session={lastSession} closeSession={closeSession} replaySession={replaySession} resetCard={function(c){sv(c);}} back={function(){stopBGM();sSP(null);sT("games");}}/>);}
  if(sp==="matchE"){if(!lastSession)playBGM("bgm_speed");return pg(<SpeedMatch mode="easy" u={u} session={lastSession} closeSession={closeSession} replaySession={replaySession} done={function(mk,res,xp,mistakes){stopBGM();return gameSession(mk,res,xp,mistakes);}} back={function(){stopBGM();sSP(null);sT("games");}}/>);}
  if(sp==="wfall"){if(!lastSession)playBGM("bgm_wfall");return pg(<WordFall u={u} session={lastSession} closeSession={closeSession} replaySession={replaySession} done={function(mk,res,xp,mistakes){stopBGM();return gameSession(mk,res,xp,mistakes);}} back={function(){stopBGM();sSP(null);sT("games");}}/>);}
  ...
  if(sp==="service")return pg(<WorldDay key="service" world="service" u={u} session={lastSession} closeSession={closeSession} replaySession={replaySession} done={function(sc,tot,xp,mistakes,extra){return worldDone("service",sc,tot,xp,mistakes,extra);}} back={function(){sSP("waygates");}}/>);
  if(sp==="opening")return pg(<WorldDay key="opening" world="opening" u={u} session={lastSession} closeSession={closeSession} replaySession={replaySession} done={function(sc,tot,xp,mistakes,extra){return worldDone("opening",sc,tot,xp,mistakes,extra);}} back={function(){sSP("waygates");}}/>);
