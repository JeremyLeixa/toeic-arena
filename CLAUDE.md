...
src/
  ...
  lib/                 — PURE helpers, no JSX, importable by tests. util, device, audio,
                          ...
                          des Waygates composées depuis P3/P4/P7, par monde) + officeGrades (grades,
                          réputation) + worlds (ce que déclare chaque monde) : ces deux-là SANS
                          données, lus par App.jsx, le hub et la tuile Games
  ...
  features/            — one folder per screen: train/ (grammar, reading, strategy),
                          ...
                          Nine to Five, Jet Lag, Front Desk et Opening Night)
  ...
  data/
    ...
    achievements.js    — 91 achievements (incl. 25 Gauntlet, 4 Word Tavern, 4 Duel, 4 Modal Council, 4 Mimic Hunt, 4 Nine to Five, 4 Jet Lag, 4 Front Desk, 4 Opening Night)
...
| The Waygates : Nine to Five, Jet Lag, Front Desk, Opening Night (modules thématiques) | `src/features/waygates/CLAUDE.md` | Un monde = même moteur (`lib/worlds.js`, `WorldDay`, `worldDone`), jamais un écran copié, aucun texte de monde dans l'écran ; réponses versées dans lisP3/lisP4/p7 **sans** `trackModSession` sur ces clés ; `office`/`travel`/`service`/`opening` hors tables de poids et en liste noire de maîtrise ; jetons du thème seulement. |
