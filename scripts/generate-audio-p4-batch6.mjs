// ── Lot 6 (2026-09-23) : copie de generate-audio-p4-batch5.mjs, même rotation de voix (impair M, pair W,
// conforme au champ `voice`), plage p4_95 → p4_102. ONLY=p4_95 → un seul talk (échantillon avant le lot).
// Clé : node --env-file=.env scripts/generate-audio-p4-batch6.mjs
...
async function main() {
  console.log("\n═══ TOEIC Arena — P4 Batch 6 (p4_95 → p4_102, lot du 2026-09-23) ═══\n");
  console.log(`Output: ${OUT_DIR}`);

  const targets = LISTENING_P4.filter((it) => voiceForId(it.id) !== null && (!process.env.ONLY || it.id === process.env.ONLY));
