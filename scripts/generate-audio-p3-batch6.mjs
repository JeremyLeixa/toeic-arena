// ── Lot 6 (2026-09-23) : copie de generate-audio-p3-batch5.mjs, mêmes voix et réglages, plage p3_89 → p3_96
// (conversations à 2 et 3 locuteurs : le combo par id%4 donne W et M distincts). ONLY=p3_89 → un seul item
// (échantillon avant le lot). Clé : node --env-file=.env scripts/generate-audio-p3-batch6.mjs
// Puis : node scripts/stitch-p3.cjs (0 crédit).
...
async function main() {
  console.log("\n═══ TOEIC Arena — P3 Batch 6 (p3_89 → p3_96, lot du 2026-09-23) ═══\n");
  console.log(`Output: ${OUT_DIR}`);

  const targets = LISTENING_P3.filter((it) => comboForId(it.id) !== null && (!process.env.ONLY || it.id === process.env.ONLY));
  console.log(`Items: ${targets.length} (expected 8, or 1 with ONLY)`);
