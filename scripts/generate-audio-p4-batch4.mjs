// ═══════════════════════════════════════════════════════════
// generate-audio-p4-batch4.mjs
// Génère les fichiers audio pour p4_46 → p4_60 (15 talks).
// Pour P4, un seul fichier par item: {id}.mp3 (le talk monologue complet).
// Les questions et options sont écrites à l'écran, jamais énoncées.
//
// Voix rotation 4 voix par id mod 4 (VOICE_A/B exclues — sex unconfirmed) :
//   id mod 4 == 2 → Sarah (W)     → p4_46, _50, _54, _58
//   id mod 4 == 3 → Adam (M)      → p4_47, _51, _55, _59
//   id mod 4 == 0 → Canadian (W)  → p4_48, _52, _56, _60
//   id mod 4 == 1 → British (M)   → p4_49, _53, _57
//
// Le `voice` field dans la data ("W"/"M") est la sex de la voix utilisée
// au runtime — il est cohérent avec la voix sélectionnée par le script.
//
// Coût estimé : ~6 000 credits (15 talks × ~400 chars).
//
// Usage (PowerShell) :
//   $env:ELEVENLABS_API_KEY="sk_xxx"
//   node scripts/generate-audio-p4-batch4.mjs
//
...
async function main() {
  console.log("\n═══ TOEIC Arena — P4 Batch 4 (p4_46 → p4_60) ═══\n");
  ...
  if (mismatch > 0) {
    console.warn(`  ⚠️  ${mismatch} mismatches detected. Audio will use the script's voice anyway.\n`);
  } else {
