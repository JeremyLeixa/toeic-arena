// ═══════════════════════════════════════════════════════════
// generate-audio-p2-batch6.mjs
// Génère TOUS les fichiers audio pour p2_191 → p2_230 (40 items × 4 = 160 files).
// Gros lot : couvre tout le spectre des fonctions de question P2 (requêtes/
// offres, statements, WH, yes/no, tag, négatives, alternatives).
//
// CRITIQUE — le préfixe "A. ", "B. ", "C. " est inclus dès la génération via
// `${letter}. ${item.opts[j]}` (bug batch2/batch3, ne peut plus se reproduire).
//
// Voix : rotation 6 voix (id mod 6), identique à batch4/batch5.
//
// Coût estimé : ~11 000 credits (160 fichiers × ~55 chars moyenne).
//
// Usage (PowerShell) :
//   $env:ELEVENLABS_API_KEY="sk_xxx"
//   node scripts/generate-audio-p2-batch6.mjs
//
...
async function main() {
  console.log("\n═══ TOEIC Arena — P2 Batch 6 (p2_191 → p2_230) ═══\n");
  console.log(`Output: ${OUT_DIR}`);

  const targets = LISTENING_P2.filter((it) => voiceForId(it.id) !== null);
  console.log(`Items: ${targets.length} (expected 40)`);
